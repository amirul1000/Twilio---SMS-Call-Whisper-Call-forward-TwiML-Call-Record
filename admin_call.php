     ...
     if($_POST){
		...
		for($i=0;$i<count($phone_arr['phone']);$i++){ 
		 ...
		 ////////////
		  ...
		  if($_POST['media']){
			  $Call_Whisper='';
			  $In_coming_Call_Message='';
			  $Answerphone = '';
			 for($k=0;$k<count($_POST['media']);$k++){
				 
				$obj = $wpdb->get_results("select * from wp_media where id='".$_POST['media'][$k]."'"); 

				 if($obj[0]->media_type=='Call Whisper'){
					 $Call_Whisper = $obj[0]->media_file;
				 }
				 /*else if($obj[0]->media_type=='In coming Call Message'){
					 $In_coming_Call_Message = $obj[0]->media_file;
				 }
				 else if($obj[0]->media_type=='Answerphone'){
					 $Answerphone = $obj[0]->media_file;
				 }*/
		...
		if(isset($_REQUEST['msg'])){
		   $say = new \Twilio\TwiML\Voice\Say($_REQUEST['msg'], [
				'voice' => 'woman'
			]);
		}
			
			/*if(!empty($In_coming_Call_Message)){
			
				$play1 = new \Twilio\TwiML\Voice\Play($In_coming_Call_Message, [
					'loop' => 1
				]);
			}*/
			
			if(!empty($Call_Whisper)){
				$play1 = new \Twilio\TwiML\Voice\Play($Call_Whisper, [
					'loop' => 2
				]);			
			}
			
			/*if(!empty($Answerphone)){
				$play3 = new \Twilio\TwiML\Voice\Play($Answerphone, [
					'loop' => 1
				]);
			}*/
			...
			$to = $phone_arr['phone'][$i];
			$from = $_POST['twillioPurchasedNumber'];
			// Make a phone call			
			/*if($_SERVER['HTTP_HOST']=='localhost'){
			  $url = "https://clickitphotobooths.co.uk/voice.xml";	
			}
			else{*/
				$url =  get_site_url()."/voice.xml";
			//}
			
			//https://www.twilio.com/docs/voice/make-calls#handle-possible-call-outcomes
			try{
				$call = $client->calls->create(
					...
					[
						"method" => "GET",
						"statusCallback" => site_url('/twillio-call-back/'),
			...
			$msg .= "Call made successfully to $to with sid: ".$call->sid."\n\n<br>";
...
<form method="post"  action="admin.php?page=call" onSubmit="return chkRequired();">
   <div class="form-group form-check">
    ...
    <ul class="list-group">
        ...
        ?>
        <li class="list-group-item">
           Enter Phone No<input type="text" name="entered_phone_no" id="entered_phone_no" value="<?php echo isset($_POST['entered_phone_no'])?$_POST['entered_phone_no']:''; ?>">
        </li>
     </ul>
      ...
      <div class="form-group">
          <label class="label" for="msg">Whisper Message:</label>
      ...
      <div class="form-group">
        ...
        <table cellspacing="3" cellpadding="3" class="table table-striped table-bordered">
            ...
            <tr>
                <td><input type="checkbox" name="media[]" value="<?php echo $c->id; ?>" checked></td>
      ...
      <div class="form-group">
        <input type="submit" class="btn btn-success label" value="Call">
      </div>
</form>
